...
const CartPage = () => {
  ...
  //   // fetch(`/api/user/cart/:${id}`,{ method:"POST",
  ...
  // const {data:{cart}}=await axios.post(`/api/user/cart/:${id}`,{ action: { type: 'increment' }} , {headers:{
  //   authorization: encodedToken
  //   }})
  //   console.log(cart)
  ...
  //   }
   ...
   async function updateQtyFromCart(id) {
    try {
      const {
        data: { cart },
      } = await axios.post(
     ...
     {state.cart.map(item=>{
        ...
        <div class="card-horizontal-column flex-vt flex-wrap">
          ...
          <div class="card__text-wrapper-1 flex-hz">
            ...
            <span class="flex-hz"><i class="fa fa-plus card-icon-plus" onClick={(e)=>updateQtyFromCart(item._id)}></i>
              <p class="card-text">{item.qty}</p>
              <i class="fa fa-minus card-icon-plus"></i>
            </span>
          </div>
