...
const stateReducer = (state, action) => {
    switch (action.type) {
        ...
        // case 'SET_SUB_CATEGORY':return state.sortByFilters.sortBySubCategory.includes(action.payload) ? { ...state, sortByFilters: { ...state.sortByFilters, sortBySubCategory: state.sortByFilters.sortBySubCategory.filter(item => item !== action.payload) } } :
